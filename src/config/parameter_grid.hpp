...
inline void validate_grid(const ParameterGrid& grid) {
    if (grid.columns.empty()) throw std::invalid_argument("ParameterGrid: columns is empty");
    if (grid.layers.empty()) throw std::invalid_argument("ParameterGrid: layers is empty");
    if (grid.cross_column_depths.empty()) throw std::invalid_argument("ParameterGrid: cross_column_depths is empty");
    if (grid.island_counts.empty()) throw std::invalid_argument("ParameterGrid: island_counts is empty");
    if (grid.seeds.empty()) throw std::invalid_argument("ParameterGrid: seeds is empty");

    if (grid.ms.empty()) throw std::invalid_argument("ParameterGrid: ms is empty");
    if (grid.rhos.empty()) throw std::invalid_argument("ParameterGrid: rhos is empty");
    ...
    if (grid.deltas.empty()) throw std::invalid_argument("ParameterGrid: deltas is empty");
    if (grid.sigma_bs.empty()) throw std::invalid_argument("ParameterGrid: sigma_bs is empty");
    if (grid.sigma_nus.empty()) throw std::invalid_argument("ParameterGrid: sigma_nus is empty");
    if (grid.ks.empty()) throw std::invalid_argument("ParameterGrid: ks is empty");
}

[[nodiscard]] inline bool is_valid_run_config(const RunConfig& cfg) {
    if (cfg.columns == 0 || cfg.layers == 0 || cfg.cross_column_depth == 0 || cfg.island_count == 0) {
        return false;
    }
    if (cfg.cross_column_depth > cfg.layers) return false;
    if (cfg.m < 0.0 || cfg.m > 1.0) return false;
    if (cfg.rho < 0.0 || cfg.rho > 1.0) return false;
    ...
    if (cfg.sigma_nu < 0.0) return false;
    if (cfg.k < 0.0 || cfg.k > 1.0) return false;
